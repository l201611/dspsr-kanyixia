  ...
  : PhaseSeriesUnloader (copy)
{
  ...
  command_line = copy.command_line;
}
...
void dsp::Archiver::set_command_line (const std::string& command)
{
...
{
  if (verbose > 2)
    cerr << "dsp::Archiver::new_Archive" << endl;
  ...
  Pulsar::Archive* result = Pulsar::Archive::new_Archive (archive_class_name);

  if (verbose > 2)
    cerr << "dsp::Archiver::new_Archive command_line='" << command_line << "'" << endl;

  if (!command_line.empty())
  {
    auto history = result->getadd<Pulsar::ProcHistory> ();
    if (history)
    {
      if (verbose > 2)
        cerr << "dsp::Archiver::new_Archive setting ProcHistory command_line='" << command_line << "'" << endl;
      history->set_command_str (command_line);
    }
  }
...
void dsp::Archiver::unload (const PhaseSeries* _profiles) try
{
  ...
  bool continue_to_unload = postprocess (archive);

  if (!continue_to_unload)
  {
    if (verbose > 2)
      cerr << "dsp::Archiver::unload not unloading '"
...
bool dsp::Archiver::postprocess (Pulsar::Archive* data) try
{
  if (script.size() == 0)
    return true;
  ...
  if (interpreter->get_status() == Pulsar::Interpreter::TestFailed)
  {
    if (verbose > 2)
      cerr << "dsp::Archiver::postprocess assertion failed in script" << endl;
    return false;
  }

  if (interpreter->get_status() == Pulsar::Interpreter::CommandFailed)
  {
    if (verbose > 2)
      cerr << "dsp::Archiver::postprocess shell command failed in script" << endl;
    return false;
  }

  if (interpreter->get_status() == Pulsar::Interpreter::Good)
  {
    if (verbose > 2)
      cerr << "dsp::Archiver::postprocess script finished successfully" << endl;
  }
...
try 
{
  ...
  if (!archive)
    throw Error (InvalidParam, "dsp::Archiver::add Pulsar::Archive", "no Archive");

  if (!phase) 
    throw Error (InvalidParam, "dsp::Archiver::add Pulsar::Archive", "no PhaseSeries");
...
try
{
  ...
  if (!archive)
    throw Error (InvalidParam, "dsp::Archiver::set Pulsar::Archive", "no Archive");

  if (!phase)
    throw Error (InvalidParam, "dsp::Archiver::set Pulsar::Archive", "no PhaseSeries");
...
try
{
  ...
  if (archive_dedispersed)
  {
    ...
    corrected->get_relative()->set_measure( phase->get_dispersion_measure() );
    corrected->get_relative()->set_reference_frequency( phase->get_centre_frequency() );
...
try
{
  ...
  if (zeroes)
  {
    if (verbose > 2 || !unload_profiles_with_zero_hits)
      cerr << "dsp::Archiver::set Pulsar::Profile " << zeroes 
                 << " out of " << nbin << " bins with zero hits" << endl;

    if (!unload_profiles_with_zero_hits)
      throw Error (InvalidParam, "dsp::Archiver::set Pulsar::Profile",
                   "%u of %u bins have zero hits", zeroes, nbin);

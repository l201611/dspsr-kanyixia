...
namespace CUDA
{
  ...
  protected:
    ...
    CUDA::bin* lagbinplan = nullptr;

    //! Copy of binplan on device
    CUDA::bin* d_binplan = nullptr;

    //! Send the binplan to GPU
    void send_binplan ();

    //! memory for temp results on the device
    float* d_lagdata = nullptr;

    unsigned current_turn = 0;
    unsigned last_ibin = 0;

    //! Get the lag data from GPU
    // TODO the lag->frequency conversion could be done on GPU
    void get_lagdata ();

...
namespace dsp
{
  ...
  */
  ...
  public:
    ...
    //! Fold::choose_nbin will print a warning if nbin is too large
    static bool bin_width_warning;
    ...
    void set_input (const TimeSeries*);

    //! Overload Transformation::set_input to set engine output
    void set_output (PhaseSeries*);

    //! Return an appropriate number of pulse phase bins
    unsigned choose_nbin (bool quiet = !bin_width_warning) const;
  ...
  protected:
    ...
    virtual void fold (uint64_t nweights, const uint16_t* weights,
    ...
    /*! For example, for CyclicFold, this is nlag */
    virtual unsigned get_ndat_lost_at_end () const { return 0; }
  ...
  private:
    ...
    Pulsar::Predictor* get_folding_predictor (const Pulsar::Parameters*, const Observation*);
  ...
  public:
    ...
    //! Set the input TimeSeries
    virtual void set_input (const TimeSeries* input) { in = input; }

    //! Set the output PhaseSeries
    virtual void set_profiles (PhaseSeries* _out) { out = _out; }
    ...
    /*!
       This method was added as a more efficient way of setting up the bin plan all in one go,
       rather than through repeated redundant calculations as was previously done using set_bin.
    */
  ...
  protected:
    ...
    Reference::To<const TimeSeries> in;

    // Copy of output
    Reference::To<PhaseSeries> out;

    float* output = nullptr;
    unsigned output_span = 0;

    const float* input = nullptr;
    unsigned input_span = 0;

    unsigned* hits = nullptr;
    unsigned hits_nchan = 0;
    bool zeroed_samples = false;

    unsigned ndat_fold = 0;
    uint64_t idat_start = 0;
    ...
    //! Set true after device and host profiles are synchronized
    /*! For CyclicFold, the periodic correlation is converted to a periodic spectrum */

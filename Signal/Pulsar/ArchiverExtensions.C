...
void dsp::Archiver::pack (dspReduction* dspR, Operation* operation)
{
  ...
  // save for SpectralKurtosis extension
  if (skestimator)
    spectral_kurtosis = skestimator;
}

void dsp::Archiver::pack (Integration* subint)
{
  if (verbose > 2)
    cerr << "dsp::Archiver::pack (Integration*)" << endl;

  // ////////////////////////////////////////////////////////////////////
  //
  // Spectral Kurtosis RFI mitigation extension
  //
  if (spectral_kurtosis)
  {
    if (verbose > 2)
      cerr << "dsp::Archiver::pack (Integration*) SpectralKurtosis extension" << endl;

    Pulsar::SpectralKurtosis* ext = subint -> getadd<Pulsar::SpectralKurtosis>();

    unsigned nchan = spectral_kurtosis->get_nchan();
    ext->set_nchan( nchan );

    unsigned npol = spectral_kurtosis->get_npol();
    ext->set_npol( npol );

    if (verbose > 2)
      cerr << "dsp::Archiver::pack (Integration*) SpectralKurtosis nchan=" << nchan << " npol=" << npol << endl;

    ext->set_M( spectral_kurtosis->get_M() );
    ext->set_excision_threshold( spectral_kurtosis->get_excision_threshold() );

    vector<float> data;
    spectral_kurtosis->get_filtered_sum (data);
    for (unsigned ichan = 0; ichan < nchan; ichan++)
      for (unsigned ipol = 0; ipol < npol; ipol++)
        ext->set_filtered_sum (ichan, ipol, data[ichan*npol + ipol]);

    vector<uint64_t> hits;
    spectral_kurtosis->get_filtered_hits (hits);
    for (unsigned ichan = 0; ichan < nchan; ichan++)
      ext->set_filtered_hits (ichan, hits[ichan]);

    spectral_kurtosis->get_unfiltered_sum (data);
    for (unsigned ichan = 0; ichan < nchan; ichan++)
      for (unsigned ipol = 0; ipol < npol; ipol++)
        ext->set_unfiltered_sum (ichan, ipol, data[ichan*npol + ipol]);

    ext->set_unfiltered_hits( spectral_kurtosis->get_unfiltered_hits() );

    spectral_kurtosis->reset_count();
  }
}
...
{
  if (verbose > 2)
    cerr << "dsp::Archiver::set Pulsar::DigitiserCounts isub=" << isub << endl;

  if (!hist_unpacker)
  {
    if (verbose > 2)
      cerr << "dsp::Archiver::set Pulsar::DigitiserCounts no HistUnpacker" << endl;
    return;
  }

  unsigned ndig = hist_unpacker->get_ndig();

  if (!ndig)
  {
    if (verbose > 2)
      cerr << "dsp::Archiver::set Pulsar::DigitiserCounts empty HistUnpacker ndig == 0" << endl;
    return;
  }

  unsigned nstate = hist_unpacker->get_nstate();

  if (!ndig)
  {
    if (verbose > 2)
      cerr << "dsp::Archiver::set Pulsar::DigitiserCounts empty HistUnpacker nstate == 0" << endl;
    return;
  }

  // Set up params
  dcnt->set_dig_mode("DSPSR"); // Don't know what this means...
  dcnt->set_nlev(nstate);
  dcnt->set_npthist(nstate);
  dcnt->set_diglev("FIX");
  ...
  {
    if (verbose > 2)
      cerr << "dsp::Archiver::set Pulsar::DigitiserCounts resize for isub=" << isub 
           << " because DigitiserCounts.nsub=" << dcnt->subints.size() << endl;
    dcnt->resize(isub+1);
  }
  ...
  // where it belongs
  dcnt->subints[isub].data.resize(ndig * nstate);
  ...
  for (int idig=0; idig<ndig; idig++)
  {
    ...
    for (int ipt=0; ipt<nstate; ipt++)
      dcnt->subints[isub].data[ipt + idig*nstate] = histogram[ipt];
  }

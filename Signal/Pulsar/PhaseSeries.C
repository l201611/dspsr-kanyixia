...
{
  if (verbose)
    cerr << "dsp::PhaseSeries ctor this=" << this 
...
{
  if (verbose)
    cerr << "dsp::PhaseSeries dtor this=" << this << " hits=" << hits 
         << " instances=" << instance_count << endl;
...
void dsp::PhaseSeries::resize (uint64_t nbin)
{
  if (verbose)
...
void dsp::PhaseSeries::resize_hits (uint64_t nbin)
{
  if (verbose)

...
dsp::LoadToFoldN::LoadToFoldN (LoadToFold::Config* config)
{
  if (Operation::verbose)
    cerr << "dsp::LoadToFoldN ctor" << endl;

  configuration = config;
  set_nthread (configuration->get_total_nthread());

  if (Operation::verbose)
    cerr << "dsp::LoadToFoldN ctor return" << endl;  
}
...
{
  if (Operation::verbose)
    cerr << "dsp::LoadToFoldN set_nthread" << endl;
  ...
  if (Operation::verbose)
    cerr << "dsp::LoadToFoldN set_nthread return" << endl;
}
...
{
  if (Operation::verbose)
    cerr << "dsp::LoadToFoldN set_configuration" << endl;
  ...
  if (Operation::verbose)
    cerr << "dsp::LoadToFoldN set_configuration return" << endl;
}

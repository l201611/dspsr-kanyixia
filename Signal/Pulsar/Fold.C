...
void dsp::Fold::set_engine (Engine* _engine)
{
  ...
  {
    if (verbose)
      cerr << "dsp::Fold::set_engine Engine::set_cerr" << endl;
    engine->set_cerr (this->cerr);
  }

  if (verbose)
    cerr << "dsp::Fold::set_engine done" << endl;
}
...
uint64_t dsp::Fold::bytes_storage() const
{
  ...
  return ndim * npol * nchan * sizeof(float) * choose_nbin ();
}
...
{
  if (verbose)
    cerr << "dsp::Fold::get_output this=" << this << " engine=" << (void*) engine.ptr() << endl;
...
{
  if (verbose)
    cerr << "dsp::Fold::prepare this=" << this << " input=" << observation << endl;
  ...
  /* If folding_period is not already set and the observation is a poln calibrator,
     then use the calibrator frequency to set the folding period */
  if (folding_period == 0 && observation->get_type() == Signal::PolnCal)
  {
    double calfreq = observation->get_calfreq();
    if (verbose)
      cerr << "dsp::Fold::prepare calfreq=" << calfreq << endl;

    if (calfreq > 0.0)
    {
      set_folding_period (1.0/calfreq);
    }
  }
...
{
  MJD time = observation->get_start_time() - 864;
...
/*! By default, print a warning when the number of phase bins is not sensible. */
bool dsp::Fold::bin_width_warning = true;
...
unsigned dsp::Fold::choose_nbin (bool quiet) const
{
  if (verbose)
  ...
  if (verbose)
    cerr << "dsp::Fold::choose_nbin folding_period=" << the_folding_period << endl;
  ...
  if (verbose)
    cerr << "dsp::Fold::choose_nbin sampling_period=" << sampling_period << endl;
  ...
  if (verbose)
    cerr << "dsp::Fold::choose_nbin minimum_bin_width="
         << minimum_bin_width << " bins or " << binwidth << " seconds" << endl;

  unsigned sensible_nbin = unsigned (the_folding_period / binwidth);

  if (verbose)
    cerr << "dsp::Fold::choose_nbin sensible nbin=" << sensible_nbin << endl;
  ...
  if (power_of_two) {
    ...
    if (verbose)
      cerr << "dsp::Fold::choose_nbin largest power of 2 < nbin=" << sensible_nbin << endl;
  }
  ...
  unsigned result = sensible_nbin;

  if (requested_nbin > 1)
  {
    // the Fold::set_nbin method has been called
    if (verbose) cerr << "dsp::Fold::choose_nbin using requested nbin=" << requested_nbin << endl;
    result = requested_nbin;
  ...
  else
  {
    ...
    {
      if (verbose) cerr << "dsp::Fold::choose_nbin using maximum nbin=" << maximum_nbin << endl;
      result = maximum_nbin;
    }
    else
    {
      if (verbose) cerr << "dsp::Fold::choose_nbin using sensible nbin=" << sensible_nbin << endl;
      result = sensible_nbin;
    }
  }
...
{
  if (verbose)
    cerr << "dsp::Fold::set_input this=" << (void*) this << " (TimeSeries* =" << _input << ")" << endl;

  Transformation<TimeSeries, PhaseSeries>::set_input (_input);
  if (engine)
    engine->set_input(_input);
...
void dsp::Fold::set_output (PhaseSeries* _output)
{
  if (verbose)
    cerr << "dsp::Fold::set_output this=" << (void*) this << " (PhaseSeries* =" << _output << ")" << endl;

  Transformation<TimeSeries, PhaseSeries>::set_output (_output);
  if (engine)
    engine->set_profiles(_output);
}
...
{
  if (verbose)
    cerr << "dsp::Fold::transformation input ndat=" << input->get_ndat() << endl;
  ...
  uint64_t nweights = 0;
  const uint16_t* weights = 0;
  unsigned ndatperweight = 0;
  unsigned weight_idat = 0;

  if (weighted_input && weighted_input->get_ndat_per_weight())
  {
                      ...
                      unsigned weight_idat)
{
  ...
  if (verbose)
    cerr << "dsp::Fold::fold id=" << id << " idat_start=" << idat_start
  ...
  if (ndatperweight)
  {
    ...
    if (verbose)
      cerr << "dsp::Fold::fold id=" << id << " ndatperweight=" << ndatperweight
  ...
  if (engine)
  {
    ...
    if (engine->use_set_bins)
    {
    	...
    	for (unsigned ibin = 0; ibin < folding_nbin; ibin++)
    	{
    		hits[ibin] += engine->get_bin_hits(ibin);
    	}
      if (verbose)
      {
        cerr << "dsp::Fold::fold use_set_bins=True, folding_nbin=" << folding_nbin << " ndat_folded=" << ndat_folded << endl;
      }
    	use_set_bins = true;
    }
  }
  ...
  {
    /*
      Generate a folding plan.
      Note that ndat_folded and hits are incremented only when zeroed_samples is false.
    */

    auto nlost = get_ndat_lost_at_end();

    for (uint64_t idat=idat_start; idat < idat_end; idat++)
    {
      uint64_t effective_ndat = 0;
      if (idat_end > nlost)
        effective_ndat = idat_end - nlost;
      ...
      {
        if (!zeroed_samples && idat < effective_ndat)
        {
          hits[ibin]++;
          ndat_folded ++;
        }
  ...
  // when zeroed_samples is true, ndat_folded is zero; therefore, time_folded is zero
  double time_folded = double(ndat_folded) / get_input()->get_rate();

  if (verbose)
    cerr << "dsp::Fold::fold id=" << id << " ndat_folded=" << ndat_folded << " ndat_fold=" << ndat_fold
  ...
  // when zeroed_samples is true, time_folded is zero; therefore integration_length must be updated later
  ...
  if (engine)
  {
    ...
    // why is this only done if zeroed_samples was true (if rfi was flagged?) shouldn't it always happen?
    /* Good question! Note that the integration length is updated shortly after creating the folding plan,
      where it is incremented by time_folded, but time_folded is a multiple of ndat_folded, but ndat_folded
      is incremented only when zeroed_samples is false because in this case it is not known at the time of
      creating the folding plan if a sample will be integrated or not.  In this case, it is necssary to
      ask the engine how many samples were non-zero and therefore actually folded.
     */
    if (zeroed_samples)
    {
    	if (verbose) {
    		cerr << "Fold::fold finishing fold w/ engine. increment integration length from " << result->integration_length
  ...
  if (in->get_order() == TimeSeries::OrderFPT)
  {
    for (unsigned ichan=0; ichan<nchan; ichan++)
    {
      ...
      if (zeroed_samples && output->get_hits_nchan() == nchan)
        hits += folding_nbin;
    } // for each chan
  }
  else
  ...
  if (zeroed_samples)
  {
    /* Note that the integration length is updated shortly after creating the folding plan,
      where it is incremented by time_folded, time_folded is a multiple of ndat_folded, and ndat_folded
      is incremented only when zeroed_samples is false because, when zeroed_samples is true, it is unknown
      at the time of creating the folding plan if a sample will be integrated or not.

      When zeroed_samples is true, ndat_folded is incremented once for every time sample that is non-zero
      in every frequency channel; therefore, it is necessary to compute time_folded using ndat_folded / nchan.
     */
...
dsp::Fold::Engine::Engine()
{
  if (Operation::verbose) 
    cerr << "dsp::Fold::Engine ctor this=" << this << endl;
}

dsp::Fold::Engine::~Engine()
{
  if (Operation::verbose) 
    cerr << "dsp::Fold::Engine dtor this=" << this << endl;
}

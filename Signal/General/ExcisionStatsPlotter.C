...
{
  char label[64];
  snprintf (label, 64, "Low states (in %d pts)",
...
bool dsp::ExcisionStatsPlotter::special (unsigned imin, unsigned imax, float& ymax)
{  
  ...
  for (unsigned iwt=0; iwt<ndat_per_weight; iwt++)
    plot_theory[iwt] = plot_value(theory[iwt*ndim] * nweights * ndim);

...
/* ***********************************************************************
   ...
   factor     - the returned FFT length is a multiple of this factor
   verbose    - how noisy?
...
int64_t optimal_fft_length (uint64_t nbadperfft, uint64_t nfft_max, unsigned factor, char verbose)
{
  ...
  /* keep doubling factor until it is larger than nbadperfft */
  nfft_min = factor;
  while (nbadperfft > nfft_min)
    nfft_min *= 2;

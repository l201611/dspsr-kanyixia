...
void dsp::LoadToFil::construct () try
{
  ...
  */
  ...
  /* 26 Sept 2023 - Willem van Straten 
    I removed a block of code that was preceded by the following message:

    // if PSRFITS input, set block to exact size of FITS row
    // this is needed to keep in sync with the callback

    The need for the callback that this refers to was removed with the following commit
    
    commit 4285cb3927399387b5596e0a9c90964cbc38b9ea
    Author: Willem van Straten <vanstraten.willem@gmail.com>
    Date:   Wed Aug 12 22:23:38 2020 +1000

        enable multiple rows to be loaded/unpacked with arbitrary byte offsets    

    More importantly, this type of constraint is the reason for the Input::resolution attribute.
  */
  ...
  if (!obs->get_detected())
  {
    ...
    if ( config->filterbank.get_nchan() )
    {
      ...
      {
        if (verbose)
          cerr << "digifil: using TFP filterbank" << endl;
    ...
    {
      if (verbose)
        cerr << "digifil: creating detection operation (npol=" << config->npol << ")" << endl;
  ...
  if (filterbank) {
        ...
        source->get_block_size())
    {
      cerr << "digifil: increasing data block size from " 
        ...
        << " samples" << endl;
      source->set_block_size( filterbank->get_minimum_samples() );
    }
  }

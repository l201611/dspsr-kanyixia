...
void CUDA::FilterbankEngine::setup (dsp::Filterbank* filterbank)
{
  ...
  output_nchan = filterbank->get_nchan();
...
void CUDA::FilterbankEngine::copy_response (const dsp::Response* response)
{
  unsigned nchan = response->get_nchan();
  unsigned ndat = response->get_ndat();
  unsigned ndim = response->get_ndim();

  if (nchan != output_nchan)
    throw Error (InvalidState, "CUDA::FilterbankEngine::copy_response",
                  "response nchan=%u != filterbank nchan=%u", nchan, output_nchan);

  if (ndat != freq_res)
    throw Error (InvalidState, "CUDA::FilterbankEngine::copy_response",
                  "response nchan=%u != filterbank freq_res=%u", ndat, freq_res);

  assert( ndim == 2 ); // complex

  unsigned mem_size = nchan * ndat * ndim * sizeof(cufftReal);

  DEBUG("CUDA::FilterbankEngine::copy_response allocating "<< mem_size << " bytes for Response kernel");

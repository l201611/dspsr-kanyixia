...
void usage ()
{
  cout << "passband - plot passband\n"
    ...
    " -P         detect the input (no further spectral resolution required) \n"
...
int main (int argc, char** argv) try {
  ...
  // detect the input voltages
  bool detect_input = false;
  ...
  static const char* args = "1bB:c:dD:f:F:G:g:hiIj:lmn:PpRr:S:T:t:svV";
  ...
  while ((c = getopt(argc, argv, args)) != -1)
    ...
    case 'F':
    {
      float min, max;
      char dummy;
      sscanf (optarg, "%f%c%f", &min, &dummy, &max);
  ...
  Reference::To<dsp::TimeSeries> voltages = new dsp::WeightedTimeSeries;
  ...
  if (detect_input)
  {
    ...
    dsp::Detection* detection = new dsp::Detection;
    detection -> set_input (voltages);
    detection -> set_output (voltages);
  ...
  if (width_pixels && height_pixels)
    ...
    if (bandwidth != 0)
    {
      cerr << "passband: over-riding bandwidth"
              " old=" << source->get_info()->get_bandwidth() <<
              " new=" << bandwidth << endl;
      source->get_info()->set_bandwidth (bandwidth);
    }

    if (centre_frequency != 0)
    {
      cerr << "passband: over-riding centre_frequency"
              " old=" << source->get_info()->get_centre_frequency() <<
              " new=" << centre_frequency << endl;
      source->get_info()->set_centre_frequency (centre_frequency);
    }
    ...
    unsigned real_complex = 2 / source->get_info()->get_ndim();
      
    unsigned block_size = ffts * nchan * real_complex;

    if (integrate_seconds && source->get_info()->get_detected())
    {
      cerr << "passband: input data are detected" << endl;
      block_size = integrate_seconds * source->get_info()->get_rate();
    }
	...
	break;
      ...
      bool finished_integration = integrate_seconds > 0 && passband->get_integration_length() > integrate_seconds;

      if (finished_integration || source->end_of_data())
      {

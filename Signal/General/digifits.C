...
int main (int argc, char** argv) try
{
  ...
  config->application_name = "digifits";
...
void parse_options (int argc, char** argv) try
{
  ...
  arg = menu.add (&config->filterbank, 
      &dsp::Filterbank::Config::set_freq_res, 'x', "nfft");
  arg->set_help ("set backward FFT length in voltage filterbank");

  arg = menu.add (config->coherent_dedispersion, "do_dedisp", "bool");
  ...
  arg = menu.add (config->tscrunch_factor, "tscr", "nsamp");
  arg->set_help ("decimate in time by this factor");

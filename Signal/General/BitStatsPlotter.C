...
double dsp::BitStatsPlotter::plot_value(double yval) const
{
  if (logarithmic_yscale)
  {
    return (yval > 0) ? log10(yval) : 0;
  }
  else
  {
    return yval;
...
void dsp::BitStatsPlotter::label()
{
  ...
  if (colour && data->has_input()
      && data->get_input()->get_state()==Signal::Analytic
      && data->get_ndim_per_digitizer() == 1)
  {
    cpgsci (colours[0]);
    cpgmtxt("T", -1.5, 0.05, 0.0, "In-phase");
    cpgsci (colours[1]);
    cpgmtxt("T", -1.5, 0.95, 1.0, "Quadrature");

    if (colours.size() > 2 && colours[0] != colours[2])
    {
      cpgsci (1);
...
void dsp::BitStatsPlotter::plot ()
{
  ...
  if (data->has_input())
    npol = data->get_input()->get_npol();

  if (npol == 2)
  {
    // plot the labels once for both
...
void dsp::BitStatsPlotter::plot (unsigned ichan, unsigned ipol)
{
  ...
  for (iplot=0; iplot < nplot; iplot++)
  {
    ...
    if (idig[iplot] == -1)
      throw Error (InvalidState, "dsp::BitStatsPlotter::plot",
		        "No digitizer channel matches ichan=%d ipol=%d ioff=%d", ichan, ipol, iplot);
    ...
    if (to_zap.size())
    { 
      cerr << "dsp::BitStatsPlotter::plot zapping histogram bins";
  ...
  if (!special (imin, imax, ymax))
  {
    ...
    // special 16-bit case to deal with large numbers
    if (data->has_input() && data->get_input()->get_nbit() == 16)
    {
      int offset = 32768;
      cpgswin (imin-offset, imax-offset, ymax*hist_min, ymax);
    }
    else
    {
  ...
  cpgbox  ("bcnst",0.0,0,ybox.c_str(),0.0,0);

  bool label_chan_pol = true;
  if (data->has_input())
  {
    auto input = data->get_input();
    if (input->get_npol() == 1 && input->get_nchan() == 1)
      label_chan_pol = false;
  }

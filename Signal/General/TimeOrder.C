...
void dsp::TimeOrder::transformation(){
  ...
  if( !input->get_swap() ){
	...
	for( unsigned ipol=0; ipol<input->get_npol(); ipol++){
	  ...
	  if( verbose )
	    fprintf(stderr,"dsp::TimeOrder::transformation(): idim=%d ichan=%d ipol=%d in=%p out = %p+%ld = %p as rapid=%s\n",
	  ...
	  if( verbose )
	    fprintf(stderr,"dsp::TimeOrder::transformation() got idim=%d ichan=%d ipol=%d input->data=%p in=%p diff=%ld output->data=%p out=%p diff=%ld\n",
		    idim, ichan, ipol,

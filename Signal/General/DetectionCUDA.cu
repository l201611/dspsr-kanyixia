...
/***************************************************************************
 *
 *   Copyright (C) 2010-2025 by Willem van Straten and Andrew Jameson
                            ...
                            uint64_t ndat, unsigned nchan)
{
  ...
  {
    const uint64_t input_offset = (ichan * input_span * COHERENCE_NPOL) + i;
    ...
    output_base[(ichan * output_span) + i] = r;
  }
}
                            ...
                            unsigned ndat, unsigned nchan)
{
  ...
  for (unsigned ichan=blockIdx.y; ichan < nchan; ichan += gridDim.y)
  {
    const unsigned input_offset = (ichan * input_span * COHERENCE_NPOL) + i;

    const float2 p0 = input_base[input_offset];
    const float2 p1 = input_base[input_offset + input_span];

    float2 s0, s1;
    COHERENCE2(s0,s1,p0,p1);

    const unsigned output_offset = (ichan * output_span * COHERENCE_NPOL) + i;

    output_base[output_offset] = s0;
    output_base[output_offset + output_span] = s1;
  }
}
                            ...
                            unsigned ndat, unsigned nchan)
{
  ...
  for (unsigned ichan=blockIdx.y; ichan < nchan; ichan += gridDim.y)
  {
    ...
    uint64_t output_offset = (ichan * output_span * 4) + i;
					 ...
					 dsp::TimeSeries* output)
{
  ...
  if (ndim == 1)
  {
    ...
    coherence1<<<blocks,threads,0,stream>>>(input_base, input_span,
                                            output_base, output_span, ndat, nchan);
  }
  else if (ndim == 2)
  {
...
// dubious about the correctness here... TODO AJ

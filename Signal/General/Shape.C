...
void dsp::Shape::resize (unsigned _npol, unsigned _nchan, unsigned _ndat, unsigned _ndim)
{
  if (verbose)
    cerr <<
      "dsp::Shape::resize"
...
void dsp::Shape::verify_dataspace()
{
  if (buffer == nullptr)
    throw Error (InvalidState, "dsp::Shape::verify_dataspace", "buffer == nullptr");

  if (npol == 0)
    throw Error (InvalidState, "dsp::Shape::verify_dataspace", "npol == 0");

  if (nchan == 0)
    throw Error (InvalidState, "dsp::Shape::verify_dataspace", "nchan == 0");

  if (ndat == 0)
    throw Error (InvalidState, "dsp::Shape::verify_dataspace", "ndat == 0");

  if (ndim == 0)
    throw Error (InvalidState, "dsp::Shape::verify_dataspace", "ndim == 0");

  unsigned required = offset*(npol-1) + nchan*ndat*ndim;
  if (bufsize < required)
    throw Error (InvalidState, "dsp::Shape::verify_dataspace", "bufsize=%u < required=%u", bufsize, required);
}

...
void dsp::TFPFilterbank::custom_prepare ()
{
  ...
  else if (npol == 2)
  {
    ...
    {
      if (verbose)
        cerr << "dsp::TFPFilterbank::custom_prepare pscrunch enabled" << endl;
    ...
    {
      if (verbose)
        cerr << "dsp::TFPFilterbank::custom_prepare interleaving" << endl;

      output->set_state (Signal::PPQQ);
    }
  }
  else
    throw Error (InvalidState, "dsp::TFPFilterbank::custom_prepare", "cannot handle npol=%u", npol);
}

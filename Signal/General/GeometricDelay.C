...
bool dsp::GeometricDelay::match (const Observation* obs)
{
  ...
  */  
  ...
  unsigned nchan = 1;    // a single, undivided band of baseband data
  ...
  auto phasors = reinterpret_cast<complex<float>*>(datptr);
  ...
  phasors = reinterpret_cast<complex<float>*>(datptr);
  for (unsigned idat=0; idat<ndat; idat++)
...
int64_t dsp::GeometricDelay::get_delay (unsigned ichan, unsigned ipol) const
{
  return delay[ipol];
}

//! Return the geometric delay for the given polarization
int64_t dsp::GeometricDelay::get_delay_range (unsigned schan, unsigned echan, unsigned ipol) const
{
  return delay[ipol];
}

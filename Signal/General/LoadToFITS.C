/***************************************************************************
 *
 *   Copyright (C) 2015 - 2023 by Matthew Kerr and Willem van Straten
...
void dsp::LoadToFITS::construct () try
{
  ...
  */
      ...
      source->get_info()->get_machine() == "FITS")
  {
    ...
    if (!manager)
      throw Error (InvalidState, "dsp::LoadToFITS::construct", "Source is not of type IOManager - cannot apply_FITS_scale_and_offset");
  ...
  if (!config->dedisperse && source->get_order_supported (config->order))
    source->set_output_order (config->order);

  // get basic information about the observation

  Observation* obs = source->get_info();
  ...
  if (config->tscrunch_factor > 0)
  {
    cerr << "digifits: requested tscr=" << config->tscrunch_factor << " rate=" << rate << endl;
    // set the desired sampling interval after integrating tscrunch_factor input time samples
    tsamp = config->tscrunch_factor / rate;
  }
  else
  {
  ...
  if (fb_nchan > 0)
  {
    ...
    if (config->tscrunch_factor > 0)
    {
      // set the desired sampling interval after integrating tscrunch_factor time samples post channelization
      tsamp *= effective_channelisation;
    }

    // Strategy will be to tscrunch from Nyquist resolution to desired reso.
    // voltage samples per filterbank sample
    samp_per_fb = tsamp * rate;
    if (verbose)
  ...
  {
    samp_per_fb = 1.0;
    tres_factor = round(rate * tsamp);
    tsamp = tres_factor/factor * 1/rate;
    nsample = config->nsblk * tres_factor;
  }

  cerr << "digifits: actual tsamp=" << tsamp << " (tscrunch=" << tres_factor << ")" << endl;
  if (verbose)
  ...
  TimeSeries* timeseries = source_output;
  ...
  if (!obs->get_detected())
  {
    ...
    if ((config->coherent_dedispersion) && (config->dispersion_measure != 0.0))
    {
    ...
    if (config->filterbank.get_nchan() > 1)
    {
      ...
      if (config->coherent_dedispersion && kernel)
    ...
    if (config->coherent_dedispersion && config->filterbank.get_convolve_when() != Filterbank::Config::During)
    {
  ...
  if ( config->dedisperse  || (config->coherent_dedispersion && (config->fscrunch_factor > 1) ) )
  {
    if (verbose)
    ...
    if (config->coherent_dedispersion && (config->fscrunch_factor > 1))
  ...
  // "subint", so we can take advantage of this to store the exact
...
void dsp::LoadToFITS::prepare () try
{
  ...
  if (Operation::verbose)
    cerr << "dsp::LoadToFITS::prepare coherent_dedispersion=" 
         << config->coherent_dedispersion << endl;

  unsigned freq_res = config->coherent_dedispersion? kernel->get_frequency_resolution() : config->filterbank.get_freq_res();
  if (freq_res == 0) 
    freq_res = 1;
  ...
  if (thread_id == 0 && config->report_vitals)
  {
    app = config->application_name;

    if (config->coherent_dedispersion && kernel)
      cerr << app << ": dedispersion filter length=" << kernel->get_ndat ()
          << " (minimum=" << kernel->get_minimum_ndat () << ")"
          << " complex samples" << endl;

    if (config->filterbank.get_nchan())
      cerr << app << ": creating " << config->filterbank.get_nchan()
          << " by " << freq_res << " back channel filterbank" << endl;
    else
      cerr << app << ": processing " << source->get_info()->get_nchan() << " channels" << endl;
  }

  convolution_block_constraints (this, config.ptr(), app);
}
catch (Error& error)
{

...
void CUDA::ConvolutionEngineSpectral::copy_response (const dsp::Response * response)
{
  ...
  if (dsp::Operation::verbose)
    cerr << "CUDA::ConvolutionEngineSpectral::copy_response response: "
  ...
  {
    if (dsp::Operation::verbose)
      cerr << "CUDA::ConvolutionEngineSpectral::copy_response kernels_size=" << kernels_size << " need_size=" << need_size << endl;
    ...
    if (error != cudaSuccess)
    {
      throw Error (InvalidState, "CUDA::ConvolutionEngineSpectral::copy_response",
  ...
  if (dsp::Operation::verbose)
    cerr << "CUDA::ConvolutionEngineSpectral::copy_response cudaMemcpy stream="

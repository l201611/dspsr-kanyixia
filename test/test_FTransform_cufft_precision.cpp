...
void throw_on_cufft_error (const cufftResult& result, const std::string& msg="")
{
  if (result != CUFFT_SUCCESS)
  {
    throw CUFFTError (result, "FTransform_cufft_precision", msg);
  }
}
          ...
          "[cuda][no_file][cufft_precision]")
{
  ...
  cudaError error = cudaMalloc((void**) &in_data_gpu_d, fft_size_bytes);
  ...
  cufftResult result = cufftPlanMany(
    ...
    CUFFT_C2C, howmany);
  throw_on_cufft_error(result, "cufftPlanMany(cufft_forward) failed");
  ...
  throw_on_cufft_error(result, "cufftPlan1d(cufft_backward) failed");

  result = cufftExecC2C(
    cufft_forward, in_data_gpu_d, out_data_gpu_d0, CUFFT_FORWARD
  );
  throw_on_cufft_error(result, "cufftExecC2C(cufft_forward) failed");
  ...
  throw_on_cufft_error(result, "cufftExecC2C(cufft_backward) failed");

  result = cufftDestroy(cufft_forward);
  throw_on_cufft_error(result, "cufftDestroy(cufft_forward) failed");

  result = cufftDestroy(cufft_backward);
  throw_on_cufft_error(result, "cufftDestroy(cufft_backward) failed");

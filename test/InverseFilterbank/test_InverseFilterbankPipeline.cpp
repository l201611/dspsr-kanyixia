...
class PipelineConfig {
  ...
  public:
    ...
    void setup_source (std::string file_path);
    ...
    const std::string& config_str)
{
  ...
  iss >> config->dedisp.inverse_filterbank;
}
...
void PipelineConfig::setup_config (const double dm, const double period)
{
  ...
  config->dedisp.coherent_dedispersion = true;
  config->dedisp.do_deripple = true;

  // set up inverse filterbank configuration
  dsp::InverseFilterbank::Config if_config;
  config->dedisp.inverse_filterbank = if_config;
  config->dedisp.temporal_apodization_type = "tukey";
}
...
void PipelineConfig::setup_source (std::string file_path) {
  Reference::To<dsp::File> file;
  file = dsp::File::create(file_path);
  auto manager = new dsp::IOManager;
...
)
{
  ...
  pipeline_config.setup_source(file_path);
  ...
  SECTION("InverseFilterbank works in no dedispersion case")
  {
    ...
    REQUIRE(pipeline_config.config->dedisp.inverse_filterbank.get_convolve_when() == dsp::Filterbank::Config::After);
    REQUIRE(pipeline_config.config->dedisp.inverse_filterbank.get_freq_res() == 1024);

    pipeline_config.config->dedisp.coherent_dedispersion = false;
  ...
  SECTION("InverseFilterbank works in during dedispersion case")
  {
    ...
    REQUIRE(pipeline_config.config->dedisp.inverse_filterbank.get_convolve_when() == dsp::Filterbank::Config::During);
    REQUIRE(pipeline_config.config->dedisp.inverse_filterbank.get_freq_res() == 0);

    pipeline_config.config->dedisp.coherent_dedispersion = true;

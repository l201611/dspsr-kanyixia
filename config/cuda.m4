...
[
  ...
  if test x"$with_cuda_dir" = xno; then
    ...
    if test -x $cuda_nvcc; then
      ...
      CUDA_BIN_PATH=`dirname $cuda_nvcc`
      CUDA_ROOT_PATH=`dirname $CUDA_BIN_PATH`
      CPPFLAGS="$CPPFLAGS -I$CUDA_ROOT_PATH/include"

      if test -d $CUDA_ROOT_PATH/lib64; then
        LDFLAGS="$LDFLAGS -L$CUDA_ROOT_PATH/lib64"
      fi

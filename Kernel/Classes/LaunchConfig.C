...
dim3 CUDA::LaunchConfig::get_max_grid_size ()
{
   if (device < 0)
  {
    throw Error (InvalidState, "CUDA::LaunchConfig::get_max_grid_size",
                 "not initialized");
    return dim3(0,0,0);
  }
  else
    return dim3(device_properties.maxGridSize[0], device_properties.maxGridSize[1], device_properties.maxGridSize[2]);
}

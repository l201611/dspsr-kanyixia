...
void dsp::HistUnpacker::zero_histogram ()
{
  ...
  if (verbose)
    cerr << "dsp::HistUnpacker::zero_histogram return" << endl;
}

void dsp::HistUnpacker::get_histogram (std::vector<unsigned long>& hist, unsigned idig) const
{
...
{
  DEBUG("dsp::HistUnpacker::get_histogram idig=" << idig << " expect=" << expect);
...
{
  if (verbose)
    cerr << "dsp::HistUnpacker::combine this size=" << histograms.size() << endl;
  ...
  if (verbose)
    cerr << "dsp::HistUnpacker::combine that size=" << like->histograms.size() << endl;

  if (histograms.size() == 0 && like->histograms.size() == 0)
    return;

  if (verbose)
    cerr << "dsp::HistUnpacker::combine this=" << this << " that=" << like << endl;

  if (histograms.size() == 0)
  {
    if (verbose)
      cerr << "dsp::HistUnpacker::combine copying histograms from that" << endl;
    histograms = like->histograms;
    return;
  }
  ...
  if (verbose)
    cerr << "dsp::HistUnpacker::combine adding histograms from that" << endl;

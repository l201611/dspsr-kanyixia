...
dsp::File::~File ()
{
  ...
  if (host_buffer)
  {
    cudaError_t result = cudaFreeHost (host_buffer);
    if (result != cudaSuccess)
    {
      std::cerr << "dsp::File::~File failed to free host memory: " << cudaGetErrorString (result) << std::endl;
    }
    host_buffer = nullptr;
  }
#endif
...
bool dsp::File::can_create (const char* filename)
{
  if (verbose)
    std::cerr << "dsp::File::can_create filename='" << filename << endl;

  // check if file can be opened for reading
  int fd = ::open (filename, O_RDONLY);
  if (fd < 0)
  {
    throw Error (FailedSys, "dsp::File::can_create", "cannot open '%s'", filename);
  }
  ::close (fd);

  const File::Register& registry = get_register();

  if (verbose)
    std::cerr << "dsp::File::can_create with " << registry.size()
	      << " registered sub-classes" << std::endl;

  for (unsigned ichild=0; ichild < registry.size(); ichild++)
  {
    if (verbose)
      std::cerr << "dsp::File::can_create testing " << registry[ichild]->get_name() << endl;
    ...
    try
    {
      ...
      {
        if (verbose)
          std::cerr << "dsp::File::can_create " << registry[ichild]->get_name()
                    << "::is_valid() returned true" << endl;
        return true;
      }
    } catch (Error& error) {
      if (verbose)
        std::cerr << "dsp::File::can_create failed while testing "
...
void dsp::File::set_total_samples () try
{
  get_info()->set_ndat (fstat_file_ndat());
}
catch (Error& error)
{
  throw error += "dsp::File::set_total_samples";
}
...
int64_t dsp::File::load_bytes_device (unsigned char* buffer, uint64_t bytes, void * device_handle)
{
  ...
  return bytes_read;
#else
  throw Error(InvalidState, "dsp::File::load_bytes_device", "not implemented");
#endif
}

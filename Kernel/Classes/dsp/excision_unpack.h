...
void dsp::ExcisionUnpacker::excision_unpack (
  U& unpack,
  Iter& input_data,
  float* output_data,
  uint64_t nfloat,
  ...
  unsigned nweights)
{
  ...
  for (unsigned long wt=0; wt<n_weights; wt++)
  {
    ...
    unpack.unpack (input_data, nfloat_per_weight, output_data, output_incr, n_low);
    ...
    */
    ...
    if ( unpack.bad || n_low<nlow_min || n_low>nlow_max || (weights && weights[wt] == 0) )
    {
#ifdef _DEBUG2
      cerr << "w[" << wt << "]=0 ";
#endif

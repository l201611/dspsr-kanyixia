...
namespace dsp {
  ...
  public:
    ...
    /*! If zero is a threshold, then there are an equal number of +ve and -ve states
        and no representation of the value 0; e.g. in signed 8-bit values, 
        the range [-128,127] is shifted by +0.5 to [-127.5,127.5] and there is no
        zero, only -0.5 and +0.5 on either side of it.

        If zero is a value, then [-128,127] remains [-128,127] and zero remains zero.
    */
    void set_zero_is_a_threshold (bool);
    bool get_zero_is_a_threshold () const { return zero_is_a_threshold; }

...
bool dsp::DummyFile::is_valid (const char* filename) const
{
  ...
  char first[32];
  if (fgets(first, 32, ptr) == NULL)
    throw Error (FailedCall, "dsp::DummyFile::is_valid", "fgets 32 characters");
...
void dsp::DummyFile::open_file (const char* filename)
{
  FILE *ptr = fopen(filename, "r");
  char header[4096];
  if (fread(header, sizeof(char), 4096, ptr) == 0)
    throw Error (FailedCall, "dsp::DummyFile::open_file", "fread failed");

...
void dsp::Input::set_overlap (uint64_t _overlap)
{
  // cerr << "dsp::Input::set_overlap " << _overlap << " samples" << endl;
  overlap = _overlap;
}
...
void dsp::Input::mark_output ()
{
  ...
  if (verbose)
    cerr << "dsp::Input::mark_output input_sample=" << output->input_sample <<
      " start_offset=" << start_offset << " request_offset=" << resolution_offset <<
      " request_ndat=" << block_size << " ndat=" << output->get_ndat() << endl;
...
void dsp::Input::close ()
{
  if (verbose)
    cerr << "dsp::Input::close does nothing" << endl;
}

void dsp::Input::restart ()
{
...
void dsp::Input::seek (int64_t offset, int whence)
{
  ...
  case SEEK_SET:
    ...
    if (verbose)
      cerr << "dsp::Input::seek SEEK_SET" << endl;
  ...
  case SEEK_CUR:
    ...
    if (verbose)
      cerr << "dsp::Input::seek SEEK_CUR" << endl;
  ...
  case SEEK_END:
    ...
    if (verbose)
      cerr << "dsp::Input::seek SEEK_END" << endl;
  ...
  if (verbose)
    cerr << "dsp::Input::seek [INTERNAL] resolution=" << resolution
         << " resolution_offset=" << resolution_offset << " load_sample=" << load_sample << endl;

  // ensure that the load_size attribute is properly set
  set_load_size ();
}

void dsp::Input::seek(const MJD& mjd) try
{
  seek(get_info()->get_idat(mjd), SEEK_SET);
}
catch (Error& error)
{
  throw error += "dsp::Input::seek";
}
...
void dsp::Input::seek_seconds (double seconds, int whence)
{
  ...
  seek( round(seconds * get_info()->get_rate()), whence );
}

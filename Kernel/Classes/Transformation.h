...
namespace dsp {
  ...
  private:
    ...
    //! The total number of samples processed by this Transformation
    double total_samples_processed = 0.0;
  };
}
...
//! Makes sure input & output are okay before calling transformation()
template <class In, class Out>
void dsp::Transformation<In, Out>::vchecks() try
{
  if (type == inplace) {
    if (Operation::verbose)
...
void dsp::Transformation<In, Out>::operation () try
{
  ...
  performance_metrics->update_metrics(this->input);

  if (buffering_policy)
  {
    if (Operation::verbose)
      cerr << name("operation") << " buffering policy post_transformation" << std::endl;
    buffering_policy -> post_transformation ();
  }

  if (Operation::verbose)
    cerr << name("operation") << " post_transformation callback" << std::endl;

  post_transformation.send(this);

  if (Operation::verbose)
    cerr << name("operation") << " done" << std::endl;
...
{
  if (Operation::verbose)
    cerr << name("dtor") << " this=" << this << " input=" << (void*) this->input.ptr() << " output=" << (void*) this->output.ptr() << std::endl;
}

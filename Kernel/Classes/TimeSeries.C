...
void dsp::TimeSeries::resize (uint64_t nsamples)
{
  ...
  if (nsamples == 0 && data == nullptr && buffer == nullptr)
    return;
  ...
  if (reserve_step == 0)
    throw Error (InvalidState, "dsp::TimeSeries::resize", "reserve_step == 0");
...
void dsp::TimeSeries::copy (const TimeSeries* that)
{
  if (this == that)
    return;

  DataSeries::copy(that);
  order = that->order;
  data = (float*)buffer;
}

const dsp::TimeSeries& dsp::TimeSeries::operator = (const TimeSeries& copy)
{
  TimeSeries::copy(&copy);
  return *this;
}
...
{
  if (verbose)
    cerr << "dsp::TimeSeries::internal_match(DataSeries*) this=" << this << " that=" << ds << endl;
...
{
  if (verbose)
    cerr << "dsp::TimeSeries::internal_match(TimeSeries*) this=" << this << " that=" << other << endl;
...
void dsp::TimeSeries::copy_dimensions (const Observation* copy)
{
  DataSeries::copy_dimensions( copy );

  auto like = dynamic_cast<const TimeSeries*> (copy);
  if (like)
    set_order( like->get_order() );
}

//! Copy the transient attributes of another TimeSeries instance
void dsp::TimeSeries::copy_transient_attributes (const Observation* copy)
{
  DataSeries::copy_transient_attributes(copy);

  auto like = dynamic_cast<const TimeSeries*> (copy);
  if (like)
    set_input_sample( like->get_input_sample() );
}

void dsp::TimeSeries::add (const TimeSeries* that)
{
  if (get_ndat()==0)
  {
    copy(that);
  ...
  if (!combinable (*that))
    throw Error (InvalidState, "TimeSeries::add",
		 "TimeSeries are not combinable");

  if (get_ndat() != that->get_ndat())
    throw Error (InvalidState, "TimeSeries::add",
		 "ndat=" UI64 " != " UI64, get_ndat(), that->get_ndat());
  ...
  if (order == OrderTFP)
  {
    ...
    const float* data2 = that->get_dattfp ();
  ...
  for (unsigned ichan=0; ichan<get_nchan(); ichan++)
  {
    for (unsigned ipol=0; ipol<get_npol(); ipol++)
    {
      ...
      const float* data2 = that->get_datptr (ichan, ipol);
...
void dsp::TimeSeries::multiply (float mult)
{
  if (verbose)
    cerr << "dsp::TimeSeries::multiply" << endl;
...
void dsp::TimeSeries::prepend_checks (const dsp::TimeSeries* pre, uint64_t pre_ndat)
{
  ...
  if (pre->get_nchan() != get_nchan())
    throw Error (InvalidState, "dsp::TimeSeries::prepend_checks",
                "data to be prepended nchan=%u != this->nchan=%u",
                pre->get_nchan(), this->get_nchan());
}
                                 ...
                                 uint64_t idat_start, uint64_t copy_ndat) try
{
  ...
  if (copy->get_npol() != get_npol())
    throw Error (InvalidParam, "dsp::TimeSeries::copy_data",
                "copy npol=%u > this npol=%u", copy->get_npol(), get_npol());

  if (copy->get_nchan() != get_nchan())
    throw Error (InvalidParam, "dsp::TimeSeries::copy_data",
                "copy nchan=%u > this nchan=%u", copy->get_nchan(), get_nchan());
...
uint64_t dsp::TimeSeries::get_stride () const
{
  if (order != OrderFPT)
    throw Error (InvalidState, "dsp::TimeSeries::get_stride", "order is not TFP");

...
template<typename T>
__global__ void copy_data_fpt_kernel(T * to, const T * __restrict__ from,
                                     ...
                                     uint64_t ndat, unsigned nchanpol)
{
  ...
  if (dx >= ndat)
    return;
  for (unsigned ichanpol=blockIdx.y; ichanpol<nchanpol; ichanpol+=gridDim.y)
  {
    to[ichanpol * to_stride + dx] = from[ichanpol* from_stride + dx];
  }
}
...
CUDA::TimeSeriesEngine::TimeSeriesEngine (dsp::Memory * _memory)
{
  ...
  if (!memory)
  {
    throw Error(InvalidParam, "CUDA::TimeSeriesEngine::TimeSeriesEngine", "provided memory manager was not a DeviceMemory manager");
  }
    ...
    uint64_t idat_start, uint64_t ndat)
{
  ...
  if (from_device == to_device)
    if (from_stream == to_stream)
      copy_data_fpt_same_stream (from, idat_start, ndat, nchan*npol);
    else
      copy_data_fpt_same_device (from, idat_start, ndat, nchan*npol);
  else
    copy_data_fpt_diff_device (from, idat_start, ndat, nchan*npol);
}
    ...
    uint64_t idat_start, uint64_t ndat, unsigned nchanpol)
{
  ...
  copy_data_fpt_kernel_multidim (to->get_datptr (0,0), from->get_datptr (0,0),
                                 ...
                                 idat_start, ndat, nchanpol, stream);
}
    ...
    uint64_t idat_start, uint64_t ndat, unsigned nchanpol)
{
  ...
  copy_data_fpt_kernel_multidim ((float *) from_engine->buffer, from->get_datptr (0,0),
                                 ...
                                 idat_start, ndat, nchanpol, from_stream);
                                 ...
                                 cudaStream_t stream)
{
  if (ndim == 2)
  {
    auto to_ptr   = reinterpret_cast<float2 *>(to);
    auto from_ptr = reinterpret_cast<const float2 *>(from);
    copy_data_fpt_kernel<float2><<<blocks,nthread,0,stream>>> (
      to_ptr, from_ptr + idat_start, to_stride, from_stride, ndat, nchanpol);
  }
  else
  {
    auto from_ptr = reinterpret_cast<const float *>(from);
    copy_data_fpt_kernel<float><<<blocks,nthread,0,stream>>> (
      to, from_ptr + idat_start, to_stride, from_stride, ndat, nchanpol);
  }
}

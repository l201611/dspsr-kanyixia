...
{
  if (verbose)
    cerr << "dsp::BitSeries::internal_match other->data_size=" << other->data_size << endl;
...
void dsp::BitSeries::copy_configuration (const Observation* copy)
{
  ...
  if (verbose)
    cerr << "dsp::BitSeries::copy_configuration this=" << this << " copy=" << copy << endl;

  Observation::operator=( *copy );

  if (verbose)
    cerr << "dsp::BitSeries::copy_configuration ndat=" << get_ndat() << " copy->ndat=" << copy->get_ndat() << endl;
}

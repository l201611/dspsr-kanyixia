...
void dsp::Seekable::load_data (BitSeries* data)
{
  ...
  if (get_info()->get_ndat())
  {
    ...
    {
      if (verbose)
        cerr << "dsp::Seekable::load_data end of data read_size=" << samples_left << endl;
...
uint64_t dsp::Seekable::recycle_data (BitSeries* data)
{
  ...
  if (verbose)
    cerr << "dsp::Seekable::recycle_data BitSeries input_sample=" << from->get_input_sample() <<
      "\n   start_offset=" << start_offset << endl;

  // Input::mark_output sets output->input_sample = load_sample - start_offset;
  uint64_t start_sample = from->get_input_sample() + start_offset;
  uint64_t last_sample = start_sample + from->get_ndat();

  if (verbose)
    cerr << "dsp::Seekable::recycle_data BitSeries data span "
  ...
  if (verbose)
    cerr << "dsp::Seekable::recycle_data to_recycle=" << to_recycle <<
      " load_size=" << get_load_size() << endl;
...
#if RESOLUTION_IS_CONSIDERED_MORE_CAREFULLY

  /* WvS 2025-03-25 perhaps blocks of recycled data should align on resolution boundaries */

  uint64_t remainder = recycle_bytes % resolution;
  if (remainder)
  {
    cerr << "dsp::Seekable::recycle_data TRUNCATE recycle_bytes by " << remainder
         << " (resolution=" << resolution << ")" << endl;
    recycle_bytes -= remainder;
  }

...
void dsp::DataSeries::resize (uint64_t nsamples, unsigned char*& old_buffer)
{
  ...
  if (verbose)
    cerr << "dsp::DataSeries::resize set_ndat(" << nsamples << ")" << endl;

  set_ndat( nsamples );

  if (verbose)
    cerr << "dsp::DataSeries::resize require=" << require << endl;
  ...
  if (verbose)
    cerr << "dsp::DataSeries::resize reshape" << endl;

  reshape ();

  if (verbose)
    cerr << "dsp::DataSeries::resize return" << endl;
}

void dsp::DataSeries::zero () try
{
  memory->do_zero (buffer, size);
}
catch (Error& error)
{
  throw error += "dsp::DataSeries::zero";
}

void dsp::DataSeries::reshape ()
{
  auto required_subsize = (get_ndim() * get_ndat() * get_nbit()) / 8;
  auto total_size = subsize * get_npol() * get_nchan();

  if (required_subsize > subsize || total_size > size)
  {
    subsize = required_subsize;
  }
...
{
  if (verbose)
    cerr << "dsp::DataSeries::internal_match other->size=" << other->size << endl;
...
void dsp::DataSeries::copy_configuration (const Observation* copy)
{
  ...
  if (verbose)
    cerr << "dsp::Dataseries::copy_configuration ndat=" << get_ndat() << endl;
}

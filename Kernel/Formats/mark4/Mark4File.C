...
int dsp::Mark4File::count_channels (int file_descriptor) const
{
  ...
  do{
    ...
    auto ret = read(file_descriptor, cc, 4); // reads 4 bytes at a time - size of SYNC
    if (ret <= 0)
      return 0;
...
MJD dsp::Mark4File::decode_date(uint64_t from)
{
  ...
  auto ret = read(fd, timecode, 8*channels);
  if (ret <= 8*channels)
    throw Error (FailedSys, "dsp::Mark4File::decode_date", "read %u bytes failed", 8*channels);
